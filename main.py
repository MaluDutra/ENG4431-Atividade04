...
from sopa import buscar_buque,busca_api,extrair_json_server
...
class Flor:
  ...
  def adicionar(self):
    dados = {"id":str(self.id),
             "nome":self.nome,
             "cor":self.cor,
             "preco":self.preco,
             "data":self.data,
             "especie":self.especie}
    url = "http://localhost:3000/flores"
...
def remover_flor():
  ...
  if certeza.upper() == "S":
    ...
    url = "http://localhost:3000/flores/%s"%id_remover
...
def atualizar_flor():
  ...
  if id_achado:
    dados = {"id":id_desejado,
       ...
       "data":data,
       "especie":especie} 
    url = "http://localhost:3000/flores/%s"%id_desejado
